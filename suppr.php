...
$prepare = $prepare->fetch();
 ...
 echo("
     <h1>Supprimer définitivement ce profil?</h1>
     <div class='info'>
     ...
     <input type='hidden' name='info_id'  value='".$prepare['info_id']."'>
     ...
     <a href='../private.php'>Retour à l'espace admin></a>
     </form>
     </div>
 ");
...
if (isset($_POST['info_id'])) {
           $info_id = htmlspecialchars($_POST['info_id']);
    try {
      $pdo = new PDO(DB_DRIVER . ":host=" . DB_HOST . ";dbname=" . DB_NAME . ";charset=" . DB_CHARSET, DB_LOGIN, DB_PASS, DB_OPTIONS);
      $requete = "DELETE FROM `info` 
